...
| <div id="childorder">childOrder?</div> |  Provides an order for outgoing edges at the given node in which the edges are arranged. | <pre>[ChildOrderDataProvider](childorderdataprovider)&lt;TNodeData,TEdgeData&gt;</pre> |
...
const nodeTypes = {
  ...
  const layoutOptions: TreeLayoutOptions = {
    edgeLabelPlacement: 'integrated',
    defaultSubtreePlacer: new yfiles.AssistantSubtreePlacer({ childSubtreePlacer: new yfiles.CompactSubtreePlacer() })
  }
